...
class ChecksApi:
    ...
    @staticmethod
    @allure.step('Проверка. Объект удалён. Код ответа положительный')
    ...
    @staticmethod
    @allure.step('Проверка. Количество сущностей > 0')
    def check_count_of_entities(ids):
        assert ids > 0, "Количество сущностей меньше 0"
    ...
    @staticmethod
    @allure.step('Проверка. Данные объекта обновлены. Код ответа положительный')
